...
public class DatabasePatchV3 extends DatabasePatch {
    ...
    private void migrateTable(Statement stmt, ISqlCommonConfig config, String table, String ownerField)
            ...
            while (result.next()) {
                ...
                try {
                    ...
                    logger.get()
                            .log(
                                    Level.SEVERE,
                                    "Failed to migrate legacy item data: table=" + table + ", " + ownerField + "="
        ...
        logger.get()
                .log(Level.INFO, "Item data migration complete: table={0}, migrated={1}, failed={2}", new Object[] {table, migrated, failed
                });

        if (failed > 0) {
            throw new SQLException("There are still " + failed + " item records that could not be migrated, database version not updated");
        }
    }
