...
public class PlayerProfileMigrator implements IMigrator {
    ...
    public MigrateStatus migrateData() {
        ...
        for (File file : listFiles) {
            ...
            try {
                ...
                if (!p.hasPlayedBefore()) {
                    Slimefun.logger().log(Level.INFO, "Detected data for a player who never joined the server, skipped: " + uuid);
                    total--;
                    continue;
                }
                ...
                Slimefun.logger().log(Level.WARNING, "Invalid player data filename detected: '" + file.getName() + "'");
        ...
        if (MigratorUtil.createDirBackup(playerFolder)) {
            Slimefun.logger()
                    .log(Level.INFO, "Successfully migrated {0} player data files! Old data was stored under ./data-storage/Slimefun/old_data", migratedCount);
            try {
                Files.deleteIfExists(playerFolder.toPath());
            } catch (IOException e) {
                result = MigrateStatus.FAILED;
                Slimefun.logger().log(Level.WARNING, "Failed to delete the old player data folder, please delete it manually", e);
            }
        }
