...
class PerformanceSummary {
    ...
    public void send(@Nonnull PerformanceInspector sender) {
        ...
        sender.sendMessage(ChatColor.GOLD
                + "Ticker runtime period: "
                + ChatColor.YELLOW
                + NumberUtils.roundDecimalNumber(tickRate / 20.0)
                + "s ("
                + tickRate
                + " ticks)");
        sender.sendMessage(ChatColor.GOLD + "Performance score: " + getPerformanceRating());
        sender.sendMessage("");
        ...
        summarizeTimings(totalTickedBlocks, "block", sender, items, entry -> {
            ...
            if (sender.getOrderType() == SummaryOrderType.AVERAGE) {
                return String.format(message, average + " | Total time: " + time);
            } else {
                return String.format(message, time + " | Average time: " + average);
            }
        });
