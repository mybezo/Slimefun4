    ...
    public void init() {
        ...
        try {
            ...
            if (readExecutorThread + writeExecutorThread > connectionPoolSize) {
                plugin.getLogger().log(Level.WARNING, "The block-storage connection pool size is smaller than the total read/write threads, this may cause performance issues");
            }
            ...
            if (blockStorageConfig.getBoolean("delayedWriting.enable")) {
                plugin.getLogger().log(Level.INFO, "Delayed writing has been enabled");
        ...
        try {
            ...
            if (readExecutorThread + writeExecutorThread > connectionPoolSize) {
                plugin.getLogger().log(Level.WARNING, "The profile-storage connection pool size is smaller than the total read/write threads, this may cause performance issues");
            }
